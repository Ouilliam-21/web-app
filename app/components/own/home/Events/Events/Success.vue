...
<template>
    <div class="relative size-full">
        ...
        <div v-bind="containerProps" class="size-full overflow-auto custom-scrollbar" @scroll="onScroll">
            <p v-if="!hasNext && events.events.length < 5" class="text-sm text-center text-muted-foreground m-3">You've reached the top of the list</p>
            ...
            <p v-if="events.events.length < 5" class="text-sm text-center text-muted-foreground">You've reached the bottom of the list</p>
        </div>
    </div>
</template>
